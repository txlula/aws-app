...
describe("Daily Mood Checker - Boundary Testing", () => {
  ...
  describe("Textarea Boundary Tests (additionalInput - max 1000 chars)", () => {
    ...
    it("should accept empty textarea input (lower boundary)", () => {
      cy.get("#additionalInput").should("have.value", "");

      cy.get("#additionalInput").invoke("val").should("have.length", 0);
    });
  ...
  describe("Range Input Boundary Tests (energyLevel)", () => {
    ...
    it("should accept minimum value (0)", () => {
      ...
      cy.get("#energyLevel").should("have.value", "0");

      cy.get("#energyLevelValue").should("contain.text", "0");
    });

    it("should accept maximum value (100)", () => {
      setRangeValue(100);

      cy.get("#energyLevel").should("have.value", "100");

      cy.get("#energyLevelValue").should("contain.text", "100");
    });

    it("should handle mid-range value (50)", () => {
      setRangeValue(50);

      cy.get("#energyLevel").should("have.value", "50");

      cy.get("#energyLevelValue").should("contain.text", "50");
    });
    ...
    it("should reflect UI updates when value changes", () => {
      ...
      values.forEach((val) => {
        ...
        cy.get("#energyLevelValue").should("contain.text", String(val));
      });
    });
